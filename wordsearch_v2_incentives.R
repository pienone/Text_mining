...
# dimensioni - gruppi
gr1<-c("PES", "Payment ecosystem service", "payment environmental service", "compensation", "voluntary contract")
gr2<-c("Carbon tax","FLEGT", "REDD","carbon credit") 
gr3<-c("bond", "impact bond", "conventional bond") 
gr4<-c("subsidy", "public incentive", "bank", "investment")
gr5<-c("insurance", "liability", "cost sharing", "negligence standard")


# Stemmiamo anche le parole nei gruppi
gr1<-c(stemDocument("pes"), stemDocument("payment ecosystem service"), stemDocument("payment environmental service"), stemDocument("compensation"), stemDocument("voluntary contract"), stemDocument("payment"))
gr2<-c(stemDocument("carbon tax"), stemDocument("flegt"), stemDocument("redd"), stemDocument("carbon credit")) 
gr3<-c(stemDocument("bond"), stemDocument("impact bond"), stemDocument("conventional bond")) 
gr4<-c(stemDocument("subsidy"), stemDocument("public incentive"), stemDocument("bank"), stemDocument("investment"))
gr5<-c(stemDocument("insurance"), stemDocument("liability"), stemDocument("cost sharing"), stemDocument("negligence standard"))
...
colSums(articles[,c("score_gr1", "score_gr2", "score_gr3", "score_gr4", "score_gr5")])

#per contare gli articoli ´con punteggio 0
sum(rowSums(articles[,c("score_gr1", "score_gr2", "score_gr3", "score_gr4", "score_gr5")])==0)
...
df<-articles[,c("Title", "score_gr1", "score_gr2", "score_gr3", "score_gr4", "score_gr5","docs")]
...
# per vedere la matrice gruppi-clusters
aggregate(dfcut, by=list(cluster=km$cluster), mean)


# per vedere a che gruppo del k means vengono assegnati gli articoli (dal kmeans)
